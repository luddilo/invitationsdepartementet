...
function EventManager(options) { // assumed to be a calendar
	...
	function isRangeAllowed(range, constraint_mod, overlap, event) {
		var loop = 1;
		var constraint;
		if (constraint_mod instanceof Array) {
			loop = constraint_mod.length;
		}

		for (var j = 0; j < loop; j++) {

			if (constraint_mod instanceof Array) {
				constraint = constraint_mod[j];
			}
			else {
				constraint = constraint_mod;
			}
			...
			if (constraint != null) {
				...
				for (i = 0; i < constraintEvents.length; i++) {
					if (!eventContainsRange(constraintEvents[i], range)) { // ADDED NEGATION (LUDVIG)
						anyContainment = true;
						break;
					}
				}
